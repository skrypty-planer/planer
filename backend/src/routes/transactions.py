...
def get_summary():
    user_id = get_user_id()
    if not user_id: return jsonify({'message': 'Unauthorized', 'status_code': 401})
    
    tm = TransactionManager()
    summary = tm.get_dashboard_summary(user_id)
    return jsonify({'summary': summary, 'status_code': HTTP_STATUS_CODE.OK})
...
def get_recent():
    ...
    return jsonify({'transactions': transactions, 'status_code': HTTP_STATUS_CODE.OK})
...
def get_charts():
    ...
    return jsonify({'charts': charts, 'status_code': HTTP_STATUS_CODE.OK})
...
def get_categories_breakdown():
    ...
    return jsonify({'breakdown': breakdown, 'status_code': HTTP_STATUS_CODE.OK})
...
def delete_transaction(transaction_id):
    ...
    if success:
        return jsonify({'success': True, 'status_code': HTTP_STATUS_CODE.OK})
    return jsonify({'message': 'Not found or failed', 'status_code': 404})
